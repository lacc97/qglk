...
//      { "not", TOKT_NOT },
    ...
    start_new_line(linebuf_.get_buf(), linebuf_.get_text_len());
    ...
    do_float:
        {
            ...
            for (*p = start, found_decpt = FALSE ; ; p->inc())
            {
                ...
                {
                    /* it's the exponent - if there's a sign, skip it */
                    p->inc();
                    cur = p->getch();
                    if (cur == '+' || cur == '-')
                        p->inc();

                    /* keep going until we find no more digits */
                    while (is_digit(p->getch()))
                        p->inc();
...
{
    /* get the next token */
    next_on_line(p, tok, in_embedding);
                                            ...
                                            int *in_embedding)
{
    ...
    do_normal:
        ...
        return next_on_line(p, tok, in_embedding);
    }
}
...
tc_toktyp_t CTcTokenizer::next_on_line_xlat_keep()
{
    ...
    for (;;)
    {
        ...
        do_normal:
            /* for anything else, use the default tokenizer */
            typ = next_on_line(&p_, &curtok_, &main_in_embedding_);
            ...
            case TOKT_INVALID:
                /* log an error for the invalid token */
                                             ...
                                             int force_embed_end)
{
     ...
     */
    reserve_source(line_len_ - (p_.getptr() - line_start_));
...
int CTcTokenizer::read_line_pp()
{
    ...
    start_new_line(p, linebuf_.get_text_len());
    ...
    if (!started_in_string && p_.getch() == '#' && allow_pp_)
    {
        ...
        while (str_->is_in_comment())
        {
            ...
            start_new_line(linebuf_.get_buf() + p_ofs,
                           linebuf_.get_text_len() - p_ofs);
        }
    ...
    else
    {
        ...
        else
        {
             ...
             */
            start_new_line(expbuf_.get_buf(), expbuf_.get_text_len());
        }
    }
 ...
 */
char *CTcTokenizer::read_line(int append)
{
    size_t len;
    size_t start_len;
    ...
    if (unsplicebuf_.get_text_len() != 0)
    {
        ...
        /* return the new text */
        return linebuf_.get_buf() + start_len;
    }
    ...
    /* 
     *   we've successfully read a line -- return a pointer to the start
     *   of the newly-read text 
     */
    return linebuf_.get_buf() + start_len;
}
...
void CTcTokenizer::unsplice_line(const char *new_line_start)
{
    ...
    /* 
     *   make sure the unsplice buffer is empty - we should never have to
     *   unsplice from a line more than once, because unsplicing should
     *   terminate the current line at the current point 
     */
    if (unsplicebuf_.get_text_len() != 0)
    {
        throw_internal_error(TCERR_MULTI_UNSPLICE);
        return;
    }
    ...
    /* put the remainder of the current line in the unsplice buffer */
    unsplicebuf_.append(new_line_start, linebuf_.get_text_len() - keep_len);
...
const char *CTcTokenizer::store_source(const char *txt, size_t len)
{
    ...
    /* reserve space in the source array */
    reserve_source(len);
...
/*
 *   Reserve space for text in the source array 
 */
                                        ...
                                        int append_to_expbuf)
{
    ...
    /* 
     *   If we're not in preprocessor mode, there's no need to remove the
     *   FULLY_EXPANDED flag bytes, since the tokenizer will know to skip
     *   them.  
     */
    if (!pp_only_mode_)
        return err;
     ...
     *   this information any longer.  When we're preprocessing the file,
     *   we don't want to leave these in the expanded source.  
     */
    for (src = dst = expbuf_.get_buf(), p.set(src) ; p.getch() != '\0' ; )
    {
    ...
    expbuf_.set_text_len(dst - expbuf_.get_buf());
                                        ...
                                        int append) const
{
    ...
    for (;;)
    {
        ...
        typ = next_on_line(srcbuf, &p, &tok, &in_embedding);
        if (typ == TOKT_EOF)
            break;
                                           ...
                                           int allow_defined)
{
    ...
    for (typ = next_on_line(&expsrc, &tok, &macro_in_embedding_) ;
         typ != TOKT_EOF ; )
    {
        /* 
                                          ...
                                          int add_close_quote)
{
     ...
     */
    for (src.set((char *)actual_val), in_inner_quote = FALSE, prvch = '\0' ;
                                 ...
                                 const CTcTokString *srcbuf, utf8_ptr *src)
{
    ...
    if (paren)
    {
        ...
        if (next_on_line(srcbuf, src, &tok, &macro_in_embedding_)
            != TOKT_RPAR)
        {
...
void CTcTokenizer::pragma_message()
{
    ...
    if (!pp_only_mode_ && !list_includes_mode_)
    {
        ...
        start_new_line(expbuf_.get_buf() + startofs,
                       expbuf_.get_text_len() - startofs);
...
void CTcTokenizer::pp_charset()
{
     ...
     */
    log_error(str_->get_charset_error()
...
void CTcTokenizer::pp_include()
{
    ...
    {
        /* no expansion needed - read from the original starting point */
        p_ = start;
    }
    ...
    /* 
     *   null-terminate the filename (we don't care what else is in the
     *   buffer at this point, so overwriting it isn't a problem) 
     */
    p_.setch('\0');
...
void CTcTokenizer::pp_define()
{
    ...
    if (argc != 0)
    {
        ...
        {
            /* get the next token */
            typ = next_on_line(&src, &tok, &in_embedding);
