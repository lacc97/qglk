                               ...
                               re_machine *sub_machine, int group_id)
{
    ...
    set_trans(new_machine->init, sub_machine->init,
              RE_GROUP_ENTER, group_id);
    set_trans(sub_machine->final, new_machine->final,
              RE_GROUP_EXIT, group_id);
}
 ...
 */
int CRegexSearcher::match(const char *const entire_str,
                          const char *const str, const size_t origlen,
 ...
 *   pattern.  
 */
int CRegexSearcher::search(const char *str, size_t len,
                           ...
                           int *result_len)
{
    ...
    for (p.set((char *)str) ; p.getptr() < max_start_pos ; p.inc(&len))
    {
        ...
        /* check for a match */
        matchlen = match(str, p.getptr(), len,
    ...
    re_group_register *regs)
{
     ...
     */
    return search(searchstr, searchlen, pattern, pattern->tuples,
    ...
    re_group_register *regs)
{
    ...
    /* match the string */
    return match(searchstr, searchstr, searchlen,
 ...
 *   
 *   If a pattern pattern will be used repeatedly in multiple searches or
 *   matches, it is more efficient to compile the pattern once, using
 *   compile_pattern(), and then re-use the compiled pattern for each
 *   search, since compiling a pattern takes time.  
 */
    ...
    int *result_len)
{
     ...
     *   the original string after we return 
     */
    return search(searchstr, searchlen,
