...
conversationManager: OutputFilter, PreinitObject
        ...
        for (start = 1 ; ; )
        {
            ...
            case 'convnode':
                ...
                if (respondingActor != nil)
                    respondingActor.setConvNode(convNodeTab[arg]);
                break;
    ...
    /*
     *   The global lookup table of ConvNode names.  This table is keyed by
     *   name; the value associated with each name key is the ConvNode
     *   object with that name. 
     */
    convNodeTab = static new LookupTable(32, 32)
    ...
    execute()
    {
        ...
        forEachInstance(ConvNode, { obj: convNodeTab[obj.name] = obj });
    ...
    showSuggestedTopicList(lst, asker, askee, explicit)
    {
        ...
        for (local i = 1, local len = lst.length() ; i <= len ; ++i)
        {
            ...
            for (local j = i + 1 ; j <= len ; ++j)
            {
                ...
                /* if item 'a' matches item 'b', remove item 'b' */
 ...
 *   conversation.
 *   
 *   A ConvNode object must be nested within an actor (via its 'location'
 *   property), to associate the ConvNode with the actor.  
 */
class ConvNode: ActorTopicDatabase
     ...
     *   is over.  If we do not wish to allow the conversation to end,
     *   return nil.  If we return nil, we should also display a message
     *   explaining why we're not allowing the termination; this should
     *   usually be cast in terms of the NPC saying something: "Hey! We're
     *   not through here!"
     *   
...
class TopicOrThingMatchTopic: ThingMatchTopic, TopicMatchTopic
    ...
    isMatchPossible(actor, scopeList)
    {
        ...
        return (inherited TopicMatchTopic(actor, obj)
                || inherited ThingMatchTopic(actor, obj));
    }
 ...
 *   walks away from the NPC, or the NPC gets bored and wanders off.  
 */
...
class HelloGoodbyeTopic: MiscTopic
    includeInList = [&miscTopics]
    matchList = [helloTopicObj, impHelloTopicObj, byeTopicObj, impByeTopicObj]
...
/* 
 *   a topic singleton for automatic goodbyes (the kind of conversation
 *   ending that happens when we simply walk away from an actor we're in
 *   conversation with, or when we ignore the other actor for enough turns
 *   that the actor gets bored and ends the conversation of its own
 *   volition) 
 */
impByeTopicObj: object;
...
class ActorState: TravelMessageHandler, ActorTopicDatabase
    ...
    remoteSpecialDesc(pov) { getActor().actorThereDesc; }
    ...
    beforeTravel(traveler, connector)
    {
         ...
         *   conversation
         */
        if (other != nil && traveler.isActorTraveling(other))
        {
    ...
    endConversation(actor, reason)
    {
        ...
        if ((node = ourActor.curConvNode) != nil)
        {
            ...
            conversationManager.finishResponse(ourActor,
                                               ourActor.curConvNode);
...
class ConversationReadyState: ActorState
     ...
     *   switching from the conversation to this state.  'explicit' is
     *   true if the other actor has ended the conversation explicitly by
     *   saying GOODBYE.
     *   
     ...
     *   
     *   If you want to distinguish between explicit and implicit
     *   goodbyes, you can create an ImpByeTopic for the implied goodbye
     *   (i.e., the kind where the other actor just walks away, or where
     *   we get bored of the other actor ignoring us).  The regular
     *   ByeTopic will handle explicit GOODBYE commands, and the
     *   ImpByeTopic will handle the implied kind.  
     */
    enterFromConversation(actor, explicit)
    {
        /* look for a ByeTopic in our database */
        handleTopic(actor, explicit ? byeTopicObj : impByeTopicObj,
                    byeConvType, nil);
    }
 ...
 *   conversations with a single character, speifically the player
...
class InConversationState: ActorState
    ...
    endConversation(actor, reason)
    {
        ...
        if (nxt.ofKind(ConversationReadyState))
            nxt.enterFromConversation(actor, reason == endConvBye);
    ...
    defaultGreetingResponse(actor)
    {
         ...
         */
        libMessages.alreadyTalkingTo(getActor(), actor);
    }
    ...
    takeTurn()
    {
        ...
        /* increment our boredom counter */
        actor.boredomCount++;

        /* 
         *   If we've exceeded our attention span, terminate the
         *   conversation, since we can assume that our conversational
         *   partner is no longer paying us any attention.
         *   
         *   If we're not terminating the conversation, and we have a
         *   current conversation node, and the last conversational command
         *   from the player character didn't happen on the current turn,
         *   tell the conversation node about it, in case the NPC wants to
         *   advance the conversation of its own volition.  We only do this
         *   when at least one turn has gone by without a conversational
         *   command from the PC, because we don't want to trigger NPC
         *   continuation when the PC performs an explicit conversational
         *   command.  
         */
        if (attentionSpan != nil && actor.boredomCount > attentionSpan)
        {
            /* return to the ready-for-conversation state implicitly */
            endConversation(actor.getCurrentInterlocutor(), endConvBoredom);
        }
        else
        {
...
class AccompanyingState: ActorState
    ...
    beforeTravel(traveler, connector)
    {
         ...
         *   actors 
         */
        if (accompanyTravel(traveler, connector))
        {
            /* 
...
class AccompanyingInTravelState: ActorState
     ...
     */
    sayDeparting(conn)
        { libMessages.sayDepartingWith(getActor(), leadActor); }
...
class ConvAgendaItem: AgendaItem
    isReady = (!getActor().conversedThisTurn() && inherited())
;
...
class Actor: Thing, Schedulable, Traveler, ActorTopicDatabase
    ...
    setConvNode(node)
    {
        ...
        if (dataType(node) == TypeSString)
            node = conversationManager.convNodeTab[node];
     ...
     *   conversation - if the player character initiates conversation
     *   with TALK TO, ASK, TELL, etc., we handle the conversation through
     *   our normal handlers for those commands.
     *   
     *   'state' is the ActorState to switch to for the conversation.
     *   This will normally be an InConversationState object, but doesn't
     *   have to be.  If 'state' is nil, we'll stay in our current state.
     *   
     *   'node' is a ConvNode object, or a string naming a ConvNode
     *   object.  We'll make this our current conversation node.  A valid
     *   conversation node is required because we use this to generate the
     *   initial NPC greeting of the conversation.  In most cases, when
     *   the NPC initiates a conversation, it's because the NPC wants to
     *   ask a question or otherwise say something specific, so there
     *   should always be a conversational context implied, thus the need
     *   for a ConvNode.  If there's no need for a conversational context,
     *   the NPC script code might just as well display the conversational
     *   exchange as a plain old message, and not bother going to all this
     *   trouble.  
     */
    initiateConversation(state, node)
    {
        /* if there's a state provided, switch to it */
        if (state != nil)
            setCurState(state);
    ...
    remoteSpecialDesc(pov) { curState.remoteSpecialDesc(pov); }
    ...
    executeAgenda()
    {
        ...
        if (item != nil)
        {
            ...
            {
                /* 
                 *   If any exception occurs while executing the item, mark
                 *   the item as done.  This will ensure that we won't get
     ...
     *   room containing the actor.  'pov' is the point of view of the
     *   description.  
     */
    listActorPosture(pov)
    {
    ...
    suggestTopics(explicit)
    {
        ...
        else if (explicit)
        {
            ...
            libMessages.noTopicsNotTalking;
        }
    }
    ...
    scopeList()
    {
        ...
        /* 
         *   Finally, add anything extra each item wants to add.  Note
         *   that we keep going until we've visited each element of the
         *   vector at its current length on each iteration, so if we add
         *   any new items, we'll check them to see if they want to add
         *   any new items, and so on. 
         */
    ...
    sayToActor(actor, topic, convType)
    {
        ...
        if (actor == self)
        {
            /* get the current interlocutor */
            actor = getCurrentInterlocutor();

            /* 
             *   if we're not talking to anyone, or if the person we were
             *   talking to can no longer hear us, look for a default
             *   object for a TALK TO command and use it instead as the
             *   default 
             */
...
#define verifyNotSelfInherit(msg) \
    ...
    dobjFor(TalkTo)
    {
        ...
        verify()
        {
            /* it's unlikely that I want to talk to myself */
            if (gActor == self)
                logicalRank(50, 'talking to self');
        }
        action()
        {
