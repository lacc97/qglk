...
prepareNestedAction(isReplacement, isRemapping, action)
{
     ...
     *   its enclosing command for timing.  
     */
    if (isReplacement && !action.isImplicit)
...
class MessageResult: object
    ...
    findTextSource:
        if (dataType(msg) == TypeProp)
        {
            ...
            /* remember the message property, for identification purposes */
            messageProp_ = msg;
             ...
             *   routine.  
             */
            foreach (local cur in gAction.getCurrentObjects())
            {
                ...
                if (cur != nil && cur.propDefined(msg))
                {
                    ...
                    else if (cur.propType(msg) == TypeSString)
                    {
                    ...
                    {
                        /* 
                         *   It's not a simple property pointer, so it's
                         *   simply the message we're to use.  For this
                         *   case, the source of the message is this
                         *   object, so forget about the normal action
                         *   message object and instead use the current
                         *   object.  Then proceed to evaluate the message
                         *   property as normal, which will fetch it from
                         *   the current object.  
                         */
                        msgObj = cur;
                    }
         ...
         */
        messageText_ = langMessageBuilder.generateMessage(msg);
         ...
         */
        messageText_ = langMessageBuilder.quoteMessage(messageText_);
    }
