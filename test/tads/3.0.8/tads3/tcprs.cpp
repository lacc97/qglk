...
/* 
 *   Equality/inequality comparison group.  Note that the equality
 *   operator is non-const because we must be able to change the operator
 *   - '==' in C-mode, '=' in TADS traditional mode.
 *   
 *   Note also that this is a special binary group - this one recognizes
 *   the non-keyword operators 'is in' and 'not in'.  
 */
...
CTcPrsNode *CTcParser::parse_expr_or_dstr(int allow_comma_expr)
{
    ...
    case TOKT_DSTR:
        ...
        ret = new CTPNDstr(G_tok->getcur()->get_text(),
    ...
    case TOKT_DSTR_START:
        /* parse the embedded-expression string */
        return CTcPrsOpUnary::parse_dstr_embed();

    default:
        /* for anything else, parse an expression */
        return (allow_comma_expr ? parse_expr() : parse_asi_expr());
    }
}
...
CTcPrsNode *CTcPrsOpIf::parse() const
{
    ...
    /* 
     *   parse the third part, which can be any other epxression,
     *   including a double-quoted string expression 
     */
...
CTcPrsNode *CTcPrsOpUnary::parse_dstr_embed()
{
    ...
    do_next_segment:
         ...
         *   comma operator.  This isn't necessary if there is nothing
...
CTcPrsNode *CTcPrsOpUnary::parse_primary()
{
    ...
    for (;;)
    {
        ...
        case TOKT_DSTR:
             ...
             *   -- log an error, but keep going so that we remain
             *   synchronized as long as the syntax is otherwise
             *   well-formed 
             */
            G_tok->log_error_curtok(TCERR_DQUOTE_IN_EXPR);
        ...
        case TOKT_DSTR_START:
            ...
            /* 
             *   double-quoted strings cannot be used within expressions -
             *   log an error, but keep parsing the syntax so that we
             *   remain synchronized if the file is otherwise well-formed 
             */
            G_tok->log_error_curtok(TCERR_DQUOTE_IN_EXPR);
...
CTcPrsNode *CTcPrsOpUnary::parse_inherited()
{
    ...
    default:
        ...
        /*
         *   Since we don't have an explicit superclass, we will at run-time
         *   need the method context to establish the next class in
...
int CTcSymFuncBase::write_to_sym_file(CVmFile *fp)
{
     ...
     */
    if (is_extern_ || ext_replace_ || ext_modify)
        return FALSE;
...
int CTcSymFuncBase::write_to_obj_file(CVmFile *fp)
{
    ...
    /* 
     *   If it's external, and we have no fixups, don't bother writing it
     *   to the object file - if we have no fixups, we have no references
     *   to the function, hence its presence in the object file would
