...
public:
                               ...
                               int *nodef)
    {
        ...
        CTcLibParserCmdline parser(hostifc, mk, full_name, lib_url);
...
protected:
    ...
    void scan_full_library(const char *val, const char *fname)
    {
        ...
        if (process_lib_arg(hostifc_, mk_, fname, url, &nodef))
        {
            /* 
...
parse_options:
    ...
    if ((show_banner || usage_err) && (opt_file == 0 || read_opt_file))
    {
        ...
        printf("TADS Compiler %d.%d.%d%s  "
               "Copyright 1999, 2004 Michael J. Roberts\n",
    ...
    for ( ; curarg < argc ; ++curarg)
    {
        ...
        else
        {
            ...
            {
                /* 
                 *   overwrite he '=' with a null byte, so that the filename
                 *   ends here 
                 */
                *p = '\0';
