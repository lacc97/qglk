...
#define vmgram_alt_procobj(p)    ((vm_obj_id_t)osrp4((p) + 4))
...
#define vmgram_tok_prod_obj(p)   ((vm_obj_id_t)osrp4((p) + 3))
...
#define vmgram_tok_tok_enum(p)   ((ulong)osrp4((p) + 3))
...
protected:
    ...
    static unsigned int calc_hash(VMG_ class CVmObjDict *dict,
...
public:
    /* get the global name */
    const char *get_meta_name() const { return "grammar-production/030000"; }
