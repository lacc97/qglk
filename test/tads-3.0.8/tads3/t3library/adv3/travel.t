 ...
 *   corresponding libMessages message methods.
 *   
...
class TravelMessageHandler: object
    ...
    sayArriving(conn) { libMessages.sayArriving(getNominalTraveler()); }
    sayDeparting(conn) { libMessages.sayDeparting(getNominalTraveler()); }

    /* generic local arrival message */
    sayArrivingLocally(dest, conn)
        { libMessages.sayArrivingLocally(getNominalTraveler(), dest); }
    ...
    {
        libMessages.sayArrivingThroughPassage(getNominalTraveler(), conn);
    }
    sayDepartingThroughPassage(conn)
    {
        libMessages.sayDepartingThroughPassage(getNominalTraveler(), conn);
    }

    /* arrival/departure via a PathPassage */
    sayArrivingViaPath(conn)
        { libMessages.sayArrivingViaPath(getNominalTraveler(), conn); }
    sayDepartingViaPath(conn)
        { libMessages.sayDepartingViaPath(getNominalTraveler(), conn); }

    /* arrival/departure up/down stairs */
    sayArrivingUpStairs(conn)
        { libMessages.sayArrivingUpStairs(getNominalTraveler(), conn); }
    sayArrivingDownStairs(conn)
        { libMessages.sayArrivingDownStairs(getNominalTraveler(), conn); }
    sayDepartingUpStairs(conn)
        { libMessages.sayDepartingUpStairs(getNominalTraveler(), conn); }
    sayDepartingDownStairs(conn)
        { libMessages.sayDepartingDownStairs(getNominalTraveler(), conn); }
;
...
class Traveler: TravelMessageHandler
    ...
    {
        /* 
         *   If the PC can see our destination, say nothing.  Since the
         *   traveler isn't leaving the PC's sight in this case, the
         *   traveler is never actually departing, but is simply moving
         *   around within the PC's field of view.  We'll describe the
         *   travel to the new location in the arrival message.
         */
        if (!gPlayerChar.canSee(dest))
        {
            /* ask the connector to describe the departure */
    ...
    describeArrival(origin, backConnector)
    {
        ...
        if (isActorTraveling(gPlayerChar))
        {
             ...
             */
            gPlayerChar.lookAround(gameMain.verboseMode);
        }
    ...
    describeNpcArrival(origin, backConnector)
    {
        ...
        if (backConnector != nil)
        {
             ...
             *   field of view.  In these cases, use the connector's
             *   "local arrival" message to indicate that we've moved
             *   around without actually leaving.  If we're arriving from
             *   an unseen origin, use the normal arrival message for the
             *   connector.  
             */
            if (gPlayerChar.canSee(origin))
                backConnector.describeLocalArrival(self, origin, location);
            else
...
class TravelConnector: Thing
     ...
     *   of the player character - that is, the traveler's origin is
     *   visible to the player character when we arrive in our
     *   destination.  We'll describe the travel not in terms of arriving,
     *   since the traveler was already here to start with, but rather as
 ...
 */
class SecretDoor: BasicOpenable, ThroughPassage
...
class AutoClosingDoor: Door
    dobjFor(TravelVia)
    {
        action()
        {
            ...
            makeOpen(nil);
            mainReport(&doorClosesBehindMsg, self);
        }
    }
;
...
class Enterable: TravelConnectorLink, Fixture
    /*
     *   "Enter" action and "Go Through" actions - these simply causes the
     *   actor to travel via the proxy connector.  
     */
...
class Exitable: TravelConnectorLink, Fixture
    /* 
     *   "Exit", "Go Through", and "TravelVia" actions - travel via the
     *   connector.
     */
    dobjFor(GetOutOf) remapTo(TravelVia, self)
    dobjFor(GoThrough) remapTo(TravelVia, self)

    /* define push-travel mappings */
...
class BasicLocation: Thing
    ...
    roomActorHereDesc(actor) { libMessages.actorInRoom(actor, self); }
    ...
    roomActorThereDesc(actor)
    {
        ...
        if (outer not in (nil, self))
        {
            ...
            libMessages.actorInRemoteNestedRoom(actor, self, outer, pov);
        }
        else
        {
    ...
    roomActorStatus(actor) { libMessages.actorInRoomStatus(actor, self); }
    ...
    roomListActorPosture(actor)
        { libMessages.actorInRoom(actor, self); }
    ...
    {
        if (remote == nil)
            libMessages.actorInGroupPrefix(posture, self, lst);
        else
            libMessages.actorInRemoteGroupPrefix(
                pov, posture, self, remote, lst);
    }
    actorInGroupSuffix(pov, posture, remote, lst)
    {
        if (remote == nil)
            libMessages.actorInGroupSuffix(posture, self, lst);
        else
            libMessages.actorInRemoteGroupSuffix(
                pov, posture, self, remote, lst);
    }
...
class Room: Fixture, BasicLocation, RoomAutoConnector
     ...
     *   dark, for example).
     *   
     *   By default this returns our floor, if we have one, because an
     *   actor is presumed to be in contact with the floor whenever in the
     *   room.  In some rooms, it's desirable to have certain objects in
     *   scope because they're essential features of the room; for example,
     *   a location that is part of a stairway would probably have the
    ...
    getExtraScopeItems(actor)
    {
        ...
        /* if we have a floor, consider it to be in scope */
        if ((floor = roomFloor) != nil)
            return [floor];
        else
            return [];
    ...
    /* treat an explicit GET OUT OF <ROOM> the same as OUT */
    dobjFor(GetOutOf) maybeRemapTo(gAction.parentAction == nil, Out)
;
...
class RoomPart: Fixture
    ...
    getHitFallDestination(thrownObj, path)
    {
        ...
        return gActor.location.getRoomPartLocation(self)
            .getDropDestination(thrownObj, path);
    }
...
class Floor: RoomPart
    ...
    roomActorHereDesc(actor)
    {
        ...
        if (actor.posture == standing)
            libMessages.roomActorHereDesc(actor);
        else
            libMessages.actorInRoom(actor, self);
    }
    ...
    {
        if (actor.posture != standing)
            libMessages.actorInRoomStatus(actor, self);
    }
    ...
    {
        if (remote != nil)
            libMessages.actorThereGroupPrefix(pov, posture, remote, lst);
        else if (posture == standing)
            libMessages.actorHereGroupPrefix(posture, lst);
        else
            libMessages.actorInGroupPrefix(posture, self, lst);
    }
    actorInGroupSuffix(pov, posture, remote, lst)
    {
        if (remote != nil)
            libMessages.actorThereGroupSuffix(pov, posture, remote, lst);
        else if (posture == standing)
            libMessages.actorHereGroupSuffix(posture, lst);
        else
            libMessages.actorInGroupSuffix(posture, self, lst);
    }

    /* 
     *   The message we use to describe this object prepositionally, as
     *   the destination of a throw or drop.  This should be a libMessages
     *   property with the appropriate prepositional phrase.  By default,
     *   we use the generic 'surface' message.  
     */
