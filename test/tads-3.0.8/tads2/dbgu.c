#ifdef RCSID
static char RCSid[] =
"$Header: d:/cvsroot/tads/TADS2/unix/dbgu.c,v 1.3 1999/07/11 00:46:39 MJRoberts Exp $";
#endif
...
#define sdesc_color text_normal_color
extern int watchpoint_color;
extern int breakpoint_color;
extern int filename_color;
#endif
...
int      *destlinep;
{
   ...
   if (cur < dest)
   {
      while (cur != dest)
      {
         max = linread(lin, dbgbuf, (int)dbgbufsiz);
         if (max == 0) break;
...
static void dbgsrcshow(lin, win, top, loc)
lindef   *lin;
oswdef   *win;
uchar    *top;
uchar    *loc;
{
    ...
    for (y = win->oswy1 ; y <= win->oswy2 ; ++y)
    {
        ...
        if (!lingets(lin, outbuf, sizeof(outbuf)))
            break;
        else
        {
            int l;
...
static void dbgusearch(lin, pos, toppos, win, str)
lindef   *lin;
uchar    *pos;                /* in: starting position; out: final position */
uchar    *toppos;                              /* location of top of screen */
oswdef   *win;
char     *str;
{
    int dest;
    ...
    linseek(lin, pos);
    if (!lingets(lin, dbgbuf, (int)dbgbufsiz)) return;
    
    for (;;)
    {
        lintell(lin, pos);
        if (!lingets(lin, dbgbuf, (int)dbgbufsiz)) break;
        os_strlwr(dbgbuf);
...
static void dbgulsho(ctx, buf)
dbgcxdef *ctx;
uchar    *buf;                     /* line information retrieved by dbglget */
{
    lindef    *lin;
    char       outbuf[128];
    int        cur;
    ...
    for (i = 0 ; i < 5 ; ++i) dbg_curstat[i] = DBGUHLIN;
...
/* display callback */
static void dbgudisp(ctx, str, strl)
void     *ctx;
char     *str;
int       strl;
{
    ...
    ossvpg(oldpg);                                 /* restore previous page */
}

/* display an error message */
void dbguerr(ctx, errno, msg)
dbgcxdef *ctx;
int       errno;
char     *msg;
{
    ...
    oldpg = ossvpg(1);                     /* figure out what page we're on */
    ossvpg(oldpg);                                 /* restore previous page */
    if (oldpg == 0) {                       /* if we're on the game page... */
        sprintf(err_prefix, "[TADS-%d: ", errno);
        os_printz(err_prefix);
        os_printz(msg);
        os_printz("]\n");
    }
    else
...
find_srcpos:
    ...
    for (y = win->oswy1 ; y < win->oswy2 ; ++y)
    {
        ...
        lingets(dbgsrclin, dbgbuf, (int)dbgbufsiz);
    }

    /* get and process commands */
    for (;;)
    {
        ossdbgloc(y, 2);                          /* locate cursor properly */
        ...
        case '\0':
            ...
            case CMD_F2:
                if (dbgsrclin)
                {
                    ...
                    linfind(dbgsrclin, dbgsrccsr, &objn, &ofs);
...
static void dbgubp(ctx, id, buf, bufsiz)
dbgcxdef *ctx;
int       id;
uchar    *buf;
uint      bufsiz;
...
dbgcxdef *ctx;
{
    ...
    dbglget(ctx, cmdbuf);                /* get information on current line */
    dbgulsho(ctx, cmdbuf);                          /* display current line */
    ...
    process_cmd:
        ...
        case 'c':                                           /* Call history */
            if (isupper(buf[1])) buf[1] = tolower(buf[1]);
            switch(buf[1])
            {
            case '+':
                ctx->dbgcxhstf = 0;
        ...
        case 'f':
            ...
            case 'v':
                if (atoi(buf + 2) == 0)
                {
                    ...
                    for (buf += 2 ; isspace(*buf) ; ++buf);
                    if (!(fp = osfoprb(buf, OSFTBIN)))  /* dbj: right type?? */
...
void dbguini(dbgcxdef *ctx, const char *game_filename)
{
    ...
    ossvpg(1);
#ifdef UNIX
    ossclr(0, 0, maxline, maxcol, text_normal_color);
#else
...
#ifdef UNIX
        dbgcurcolor = text_bold_color;
        dbgbpcolor = breakpoint_color;
        watch_color = watchpoint_color;
        ldesc_color = filename_color;
#endif
    ...
    for (i = 0 ; i <= maxcol && i+1 < sizeof(buf) ; ++i) buf[i] = DBGUHLIN;
    buf[i] = '\0';
 ...
 *   simply return a failure indication.  On some systems, this is used to
 *   display a "find file" dialog to allow the user to locate a source
 *   file manually when we can't find it in any of the places we know to
 *   look.
 *   
 *   Refer to the HTML TADS Win32 implementation for details on how this
 *   can implemented for friendlier behavior on a GUI system.  
 */
